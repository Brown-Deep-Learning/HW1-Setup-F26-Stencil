...
MY_CONDA_ENV_NAME="csci2470"                 # name of the conda environment
MY_CONDA_ENV_CFG_FILE="env_setup/Apple_Silicon/csci2470-silicon.yml" # name/path of the conda environment config file
...
main() {
    ...
    conda env create -n "$MY_CONDA_ENV_NAME" -f "$MY_CONDA_ENV_CFG_FILE" \
        ...
        && conda run -n "$MY_CONDA_ENV_NAME" python -m ipykernel install --user --name "$MY_CONDA_ENV_NAME" --display-name "$MY_CONDA_ENV_NAME $(python -V)" \
        && echo -e $GREEN"kernel $MY_CONDA_ENV_NAME successfully installed"$NC
}
